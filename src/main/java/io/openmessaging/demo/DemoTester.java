...
public class DemoTester {
    ...
    public static final int MSG_NUM = 1024;
    ...
    public static void main(String[] args) {
         ...
         */
        properties.put("STORE_PATH", "store");
        ...
        List<Message> messagesForTopic1 = new ArrayList<>(MSG_NUM);
        List<Message> messagesForTopic2 = new ArrayList<>(MSG_NUM);
        List<Message> messagesForQueue1 = new ArrayList<>(MSG_NUM);
        List<Message> messagesForQueue2 = new ArrayList<>(MSG_NUM);
        for (int i = 0; i < MSG_NUM; i++) {
            //注意实际比赛可能还会向消息的headers或者properties里面填充其它内容
            messagesForTopic1.add(producer.createBytesMessageToTopic("TOPIC_1", ("TOPIC_1" + i).getBytes()));
            messagesForTopic2.add(producer.createBytesMessageToTopic("TOPIC_2", ("TOPIC_2" + i).getBytes()));
            messagesForQueue1.add(producer.createBytesMessageToQueue("QUEUE_1", ("QUEUE_1" + i).getBytes()));
            messagesForQueue2.add(producer.createBytesMessageToQueue("QUEUE_2", ("QUEUE_2" + i).getBytes()));
        }
        ...
        //消费样例1，实际测试时会Kill掉发送进程，另取进程进行消费
        {
            ...
            while (true) {
                ...
                //实际测试时，会一一比较各个字段
                BytesMessage mes= (BytesMessage) message;
                Assert.assertNotNull(mes.getBody());
                Assert.assertTrue(topic!=null||queue!=null);
                if (topic != null) {
